...
class ObjectSelection:
    def __init__(self, era):
        ...
        self.FatJetPt_gg0lIncl_MaxThsh = 9999
        ...
        self.ElectronMVAId ='mvaFall17V2Iso_WP90'#(<-recommenda) # 'mvaFall17V2Iso_WP80', 'mvaFall17V2Iso_WP90' 'mvaFall17V2Iso_WPL'
    ...
    def selectElectrons(self, eventsObj):
        maskSelElectrons = (
            (eventsObj.pt > self.ElectronMinpT) & ((abs(eventsObj.eta) < 1.44) | (abs(eventsObj.eta) > 1.57)) &
            (abs(eventsObj.eta) < 2.5) &
...
class HToAATo4bProcessor(processor.ProcessorABC):
    ...
    def process_shift(self, events, shift_syst=None):
        ...
        ## Step 1: Select AK4 Jets not overlapping with the leading FatJet
        ...
        ak4Jets_nonoverlapping_FatJet = ak4Jets[mask_ak4Jets_nonoverlapping_FatJet]

        ## Step 3: Select b-tagged AK4 Jets and apply a veto on events containing b-jets
        mask_ak4Jets_bTagged      = ak4Jets_nonoverlapping_FatJet.btagDeepFlavB < self.objectSelector.Ak4JetDeepJetB_Thsh
        Ak4Jets_Bveto             = ak4Jets_nonoverlapping_FatJet[mask_ak4Jets_bTagged]

        ## Step 4: Apply the mask to select events with at least two AK4 jets
        nAk4jets_event = ak.fill_none(ak.count(Ak4Jets_Bveto.pt, axis=1), 0)  # Number of jets per event

        # Apply the mask and sort the jets by pt
        SelectedAk4Jets_bveto = ak.mask(Ak4Jets_Bveto, nAk4jets_event >= 2)
        sorted_indices = ak.argsort(SelectedAk4Jets_bveto.pt, axis=1, ascending=False)
        sorted_Ak4Jets_bveto = SelectedAk4Jets_bveto[sorted_indices]
        
        # Identify the leading and subleading jets
        leadingAk4Jet =    sorted_Ak4Jets_bveto[:, 0]
        subleadingAk4Jet = sorted_Ak4Jets_bveto[:, 1]
        
        ## Step 5: Calculate delta eta and mass for leading and subleading AK4 jets
        #ak4Jets_dEta = abs(leadingAk4Jet.eta - subleadingAk4Jet.eta)
        #ak4Jets_mass = (leadingAk4Jet + subleadingAk4Jet).mass

        ak4Jets_mass = ak.fill_none(
            (sorted_Ak4Jets_bveto[:, 0] + sorted_Ak4Jets_bveto[:, 1]).mass
        , 0)
        ak4Jets_dEta = ak.fill_none(
            abs(sorted_Ak4Jets_bveto[:, 0].eta - sorted_Ak4Jets_bveto[:, 1].eta)
        , 0)
        ...
        ak4Jets = selectAK4Jets(Jets=events.Jet, era=self.datasetInfo["era"], pT_Thsh=30)
        mask_ak4Jets_nonoverlapping_FatJet = ak4Jets.delta_r(leadingFatJet) > 0.8
        selected_ak4Jets = ak4Jets[mask_ak4Jets_nonoverlapping_FatJet]

        ## Step 3: Select b-tagged AK4 Jets and apply a veto on events containing b-jets
        # Identify jets that are b-tagged
        mask_ak4Jets_bTagged = selected_ak4Jets.btagDeepFlavB > self.objectSelector.Ak4JetDeepJetB_Thsh
        selected_Ak4Bjets = selected_ak4Jets[mask_ak4Jets_bTagged]

        # Count the number of b-tagged jets per event
        nAk4Jets_bTagged = ak.fill_none(ak.count(selected_Ak4Bjets.pt, axis=1), 0)
        #nAk4Jets_bTagged = ak.fill_none(ak.num(selected_Ak4Bjets), 0)
        
        # Create a mask to veto events containing b-jets
        #mask_nak4Bjets = nAk4Jets_bTagged <= 0
        #Ak4Jets_Bveto = ak.mask(selected_ak4Jets, mask_nak4Bjets)

        ## Step 4: Apply the mask to select events with at least two AK4 jets
        Ak4Jets_nonB         = selected_ak4Jets.btagDeepFlavB < self.objectSelector.Ak4JetDeepJetB_Thsh
        selected_NonBAk4Jets = selected_ak4Jets[Ak4Jets_nonB]

        nAk4jets_event = ak.fill_none(ak.num(selected_NonBAk4Jets), 0)  # Number of jets per event
        nAk4jet_mask = nAk4jets_event >= 2

        # Apply the mask and sort the jets by pt
        SelectedAk4Jets_bveto = ak.mask(selected_ak4Jets, nAk4jet_mask)
        #sorted_indices        = ak.argsort(SelectedAk4Jets_bveto.pt, axis=1, ascending=False)
        #sorted_Ak4Jets_bveto  = SelectedAk4Jets_bveto[sorted_indices]
        
        # Identify the leading and subleading jets
        leadingAk4Jet = ak.firsts(SelectedAk4Jets_bveto)
        subleadingAk4Jet = SelectedAk4Jets_bveto[:, 1]
        
        ## Step 5: Calculate delta eta and mass for leading and subleading AK4 jets
        ak4Jets_dEta = abs(leadingAk4Jet.eta - subleadingAk4Jet.eta)
        ak4Jets_mass = (leadingAk4Jet + subleadingAk4Jet).mass
        
        ## Step 6: Define selection criteria for loose and tight VBF cuts
        ...
        # Handle None values by filling them with False
        ...
        if "nAk4JetsbTagged" in self.sel_conditions_all_list:
            selection.add(
                "nAk4JetsbTagged",
                (nAk4Jets_bTagged <= 0)
            )
        ...
        if "nak4jets_loose" in self.sel_conditions_all_list:
            selection.add(
                "nak4jets_loose", 
                (ak4Jets_dEta > 2.2) & (ak4Jets_mass > 400) & (~ mask_jjvbf_Tight)
            )
