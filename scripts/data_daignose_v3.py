...
# For mH: right sideband (140-150 GeV for 2 bins)
mH_right_min = mH_max  # 140 GeV
mH_right_max = mH_max + 2 * mH_bin_width  # 140 + 10 = 150 GeV
...
def get_full_integral(hist2D):
    """Get integral of entire 2D histogram (full range)"""
...
def create_2D_plot(hist2D, title, filename):
    ...
    # Use "COLZ TEXT" to show bin contents
    hist_clone.Draw("COLZ TEXT")
    ...
    hist_clone.GetXaxis().SetRange(0, hist_clone.GetNbinsX() + 1)
    hist_clone.GetYaxis().SetRange(0, hist_clone.GetNbinsY() + 1)
...
for year_cat in year_cats:
    ...
    # Full range (entire histogram)
    print(f"\n--- FULL RANGE (Entire Histogram) ---")
    for sub_cat in sub_cats:
        if sub_cat in data[year_cat]:
            total = get_full_integral(data[year_cat][sub_cat])
