...
time python3 PlotHistos1D_DataVsMC_run2_ScaleQCDonly.py /eos/user/m/moanwar/htoaa/analysis/20260223_DataMC_plots/ Run2 VBFIncl  
'''
...
sIpFiles = {}
for Era in Years:
    sIpFiles[Era] = '%s/%s/%s/analyze_htoaa_stage1.root' % (sAnaDir, Era, anaSuperCat)
    #sIpFiles[Era] = '%s/%s/analyze_htoaa_stage1.root' % (sAnaDir, Era)
    # 20250612_gg0lDataMC_1, 20250613_gg0lDataMC_1, 20250617_gg0lDataMC, 20250617_gg0lDataMC_1
...
if 'VBF'       in CAT: from HistogramListForPlottingDataVsMC_Analysis_VBFMode               import *
...
for sDatasetName, YearsToRun_list in YearsToRun_dict.items():
    ...
    sOpDir  = '%s/%s/%s/%s' % (sAnaDir, sDatasetName, sOpDirNameShort, CAT)
                ...
                for yAxisScale in YaxisScaleToRun: #['linearY', ]: # ['linearY', 'logY']
                    ...
                    if len(list(MCBkg_dict.keys())) > 0:
                        ...
                        for i_, (MCBkgNameShort, MCBkg_list) in enumerate(MCBkg_dict.items()):
                            ...
                            for dataset in MCBkg_list:
                                ...
                                for Year_ in YearsToRun_list:
                                    histo_name_toUse_full = f"evt/{dataset}/{histo_name_toUse}_{systematic}"
                                    ...
                                    h_dataset = h_i if h_dataset is None else h_dataset + h_i

                                h = h_dataset if h is None else h + h_dataset
                        ...
                        for idx in idx_hBkg_sortedByIntegral:
                            label = sBkg_list[idx]
                            h = hBkg_list[idx]
                            if "QCD" in label and scale_MCBkg != 1:
                                label = f"{label} x {scale_MCBkg:.1f}"
                            hStack_list.append(h)
                            sStack_list.append(label)
                        ...
                        hBkgTot_values = np.sum([scale_MCBkg * h for h in hStack_values_list], axis=0)
                        ...
                        if nHistoDimemsions == 1: # 1-D histogram
                            #print("Stack labels order:", sStack_list)
                            #print("Colors used:", colors_toUse)
                            hep.histplot(
                                np.array([
                                    h * scale_MCBkg if ("QCD" in sBkg_list[idx] and scale_MCBkg != 1) else h for idx, h in enumerate(hStack_values_list)
                                ]),
                                bins=hStack_edges, 
                                ax=axTop, 
                                histtype='fill', 
                    ...
                    axRatio.set_ylim(yRatioLimit[0], yRatioLimit[1]) 
                    ...
                    # Create a dictionary to store histograms
                    hists_to_save = {}

                    # Add Data histogram if it exists
                    if hData is not None:
                        hists_to_save['Data'] = hData

                    # Add individual background histograms (from histos_dict)
                    for name, hist_obj in histos_dict.items():
                        if name != 'Data':
                    ...
                    if len(MCSig_list) > 0:
                        for iSig, (dataset, sig_hist) in enumerate(zip(MCSig_list, hSig_list)):
                            ...
                            # Just save the original histogram - apply scaling later when plotting
                            hists_to_save[sig_name] = sig_hist

                    # Add total background if it exists
