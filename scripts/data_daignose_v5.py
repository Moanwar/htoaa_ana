...
def create_2D_plot_region(hist2D, title, filename, x_min, x_max, y_min, y_max):
    ...
    # Get the bin edges - but force 2 GeV bins for mA
    x_edges = []
    for i in range(x_bin_min, x_bin_max + 2):
        x_edges.append(hist2D.GetXaxis().GetBinLowEdge(i))
    
    # For y-axis (mA), create bins with 2 GeV width
    y_edges = []
    current_y = y_min
    while current_y <= y_max:
        y_edges.append(current_y)
        current_y += 2  # Force 2 GeV bin width
    # Ensure we cover up to y_max
    if y_edges[-1] < y_max:
    ...
    for i in range(x_bin_min, x_bin_max + 1):
        for j in range(y_bin_min, y_bin_max + 1):
            ...
            # Find which bin in the new histogram this belongs to
            x_bin_new = i - x_bin_min + 1
            # For y, we need to find the correct bin index in the rebinned histogram
            y_low = hist2D.GetYaxis().GetBinLowEdge(j)
            y_high = hist2D.GetYaxis().GetBinUpEdge(j)
            # Find which new y bin contains this
            y_bin_new = region_hist.GetYaxis().FindBin(y_low + (y_high - y_low)/2)
            region_hist.AddBinContent(x_bin_new, y_bin_new, content)
