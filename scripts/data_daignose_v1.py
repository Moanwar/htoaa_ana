...
for year_cat in year_cats:
    ...
    legend_mH = ROOT.TLegend(0.65, 0.65, 0.9, 0.9)  # Adjusted position to accommodate longer text
    ...
    for i, sub_cat in enumerate(sub_cats):
        if sub_cat in data[year_cat]:
            ...
            if proj:
                # Calculate total events in the ROI for this projection
    ...
    for idx, (proj, total_events) in enumerate(projections):
        if idx == 0:
            ...
            proj.Draw("HIST SAME")
        # Add to legend with total events
