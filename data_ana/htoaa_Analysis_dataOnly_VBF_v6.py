...
def check_triggers_with_priority(chain, year, current_dataset_type, verbose=False):
    ...
    # Year-dependent logic
    if '2018' in year:
        # Only JetHT dataset exists - simple OR
        if fired['JetHT'] or fired['BTagCSV']:
            return True, {'keep': True, 'fired': fired}
        else:
            return False, {'keep': False, 'fired': fired, 'reason': 'No triggers fired'}
    
    else:  # 2016 or 2017 - separate datasets
        if current_dataset_type == 'JetHT':
            # JetHT dataset: keep if EITHER trigger fires
            if fired['JetHT'] or fired['BTagCSV']:
                return True, {'keep': True, 'fired': fired}
            else:
                return False, {'keep': False, 'fired': fired, 'reason': 'No triggers fired'}
        
        elif current_dataset_type == 'BTagCSV':
            # BTagCSV dataset: keep ONLY if BTagCSV fired AND JetHT did NOT fire
            if fired['BTagCSV'] and not fired['JetHT']:
                return True, {'keep': True, 'fired': fired}
            else:
                return False, {'keep': False, 'fired': fired, 'reason': 'JetHT fired or no BTagCSV'}
        
        else:
            return False, {'keep': False, 'reason': f'Unknown dataset type: {current_dataset_type}'}
        
def check_triggers_with_priority_v0(chain, year, current_dataset_type, verbose=False):
    """
    Check triggers with priority handling to avoid double-counting across datasets.
    Checks all triggers from original code, gracefully handles missing branches.
...
def process_dataset(dataset_name, output_name=None):
    ...
    for iEvt in range(nEntries):
        ...
        if '2016' in year:
            noise = noise and chain.Flag_ecalBadCalibFilter
        if not noise:
            continue
        ...
        for iFat in range(nFatJet):
            fatjet_pt = getattr(chain, 'FatJet_pt_nom', [0])[iFat] if hasattr(chain, 'FatJet_pt_nom') else 0
            ...
            fatjet_msoftdrop = getattr(chain, 'FatJet_msoftdrop_nom', [0])[iFat] if hasattr(chain, 'FatJet_msoftdrop_nom') else 0
            ...
            if hasattr(chain, 'FatJet_particleNet_TvsQCD'):
                ...
                # Add 0.001 for 'post' VFP 2016 (matching Script 2)
                threshold = 0.957 + (0.001 if 'post' in year or 'Post' in year else 0)
                if ('2018' in year and top_score > 0.970) or \
                   ('2017' in year and top_score > 0.970) or \
                   ('2016' in year and top_score > threshold):
                    if fatjet_pt > 300:
                        xFatTops.append(iFat)  # Just append, sorting not needed for veto
        ...
        for iMu in range(nMuon):
            ...
            # pT threshold: 26 GeV, plus 3 GeV for 2017 (matching Script 2)
            pt_threshold = 26.0 + (3.0 if '2017' in year else 0)
            if mu_pt <= pt_threshold: continue
            
            mu_eta = getattr(chain, 'Muon_eta', [0])[iMu] if hasattr(chain, 'Muon_eta') else 0
            if abs(mu_eta) >= 2.4: continue
            
            # Muon ID requirements (medium ID for pT <= 200, highPt ID for pT > 200)
            mu_mediumId = getattr(chain, 'Muon_mediumId', [0])[iMu] if hasattr(chain, 'Muon_mediumId') else 0
            mu_highPtId = getattr(chain, 'Muon_highPtId', [0])[iMu] if hasattr(chain, 'Muon_highPtId') else 0
            if mu_mediumId == 0 and (mu_pt <= 200 or mu_highPtId == 0): continue
            
            # Isolation requirement
            mu_miniPFRelIso_all = getattr(chain, 'Muon_miniPFRelIso_all', [0])[iMu] if hasattr(chain, 'Muon_miniPFRelIso_all') else 999
            if mu_miniPFRelIso_all >= 0.10: continue
            
            # Impact parameter requirements
            mu_dxy = getattr(chain, 'Muon_dxy', [0])[iMu] if hasattr(chain, 'Muon_dxy') else 999
            if abs(mu_dxy) >= 0.02: continue
            
            mu_dz = getattr(chain, 'Muon_dz', [0])[iMu] if hasattr(chain, 'Muon_dz') else 999
            if abs(mu_dz) >= 0.10: continue
        ...
        # 8. Electron veto (matching Script 2 from Table 3)
        hasElectron = False
        ...
        for iEle in range(nElectron):
            ...
            # pT threshold: 30 GeV, plus 5 GeV for non-2016 (matching Script 2)
            pt_threshold = 30.0 + (5.0 if '2016' not in year else 0)
            if ele_pt <= pt_threshold: continue
            
            ele_eta = getattr(chain, 'Electron_eta', [0])[iEle] if hasattr(chain, 'Electron_eta') else 0
            if abs(ele_eta) >= 2.5: continue
            
            # Exclude crack region (1.44 < |eta| < 1.57)
            if abs(ele_eta) > 1.44 and abs(ele_eta) < 1.57: continue
            
            # Electron ID requirements
            ele_mvaWP90 = getattr(chain, 'Electron_mvaFall17V2Iso_WP90', [0])[iEle] if hasattr(chain, 'Electron_mvaFall17V2Iso_WP90') else 0
            if ele_mvaWP90 == 0: continue
            
            ele_mvaWP80 = getattr(chain, 'Electron_mvaFall17V2Iso_WP80', [0])[iEle] if hasattr(chain, 'Electron_mvaFall17V2Iso_WP80') else 0
            ele_cutBasedHEEP = getattr(chain, 'Electron_cutBased_HEEP', [0])[iEle] if hasattr(chain, 'Electron_cutBased_HEEP') else 0
            if ele_mvaWP80 == 0 and (ele_pt <= 35 or ele_cutBasedHEEP == 0): continue
            
            # Impact parameter requirements
            ele_dxy = getattr(chain, 'Electron_dxy', [0])[iEle] if hasattr(chain, 'Electron_dxy') else 999
            if abs(ele_dxy) >= 0.02: continue
            
            ele_dz = getattr(chain, 'Electron_dz', [0])[iEle] if hasattr(chain, 'Electron_dz') else 999
            if abs(ele_dz) >= 0.10: continue
        ...
        vFatH_pt = getattr(chain, 'FatJet_pt_nom', [0])[xFatH] if hasattr(chain, 'FatJet_pt_nom') else 0
        ...
        vFatH_mass = getattr(chain, 'FatJet_mass_nom', [0])[xFatH] if hasattr(chain, 'FatJet_mass_nom') else 0
        ...
        vJetsLF = []  # Light-flavor jets (non b-tagged)
        vAllJets = []  # All jets for veto
        hasBJet = False
        
        nJet = getattr(chain, 'nJet', 0)
        for iJet in range(nJet):
            jet_pt = getattr(chain, 'Jet_pt_nom', [0])[iJet] if hasattr(chain, 'Jet_pt_nom') else 0
            if jet_pt <= 30: continue
            
            # Add jetId requirement (match Script 2)
            jet_jetId = getattr(chain, 'Jet_jetId', [0])[iJet] if hasattr(chain, 'Jet_jetId') else 0
            if jet_jetId != 6: continue
            
            # Add pileup ID requirement for low pT jets (match Script 2)
            jet_puId = getattr(chain, 'Jet_puId', [0])[iJet] if hasattr(chain, 'Jet_puId') else 0
            if jet_pt <= 50 and jet_puId < 4: continue
            ...
            jet_mass = getattr(chain, 'Jet_mass_nom', [0])[iJet] if hasattr(chain, 'Jet_mass_nom') else 0
            ...
            # Add to all jets collection
            vAllJets.append(vJet)
            
            # Check if b-tagged (match Script 2 logic)
            ...
            if isBJet:
                hasBJet = True
                # Don't add b-jets to light-flavor collection
            else:
                vJetsLF.append(vJet)
        
        # Apply b-jet veto (any b-jet anywhere vetoes the event)
        if hasBJet:
            continue
        
        # Check VBF veto using ALL jets (matching Script 2's VBFjj_veto)
        if len(vAllJets) < 2:
            continue
        
        # Sort all jets and take two leading for veto check
        vAllJets.sort(key=lambda j: j.Pt(), reverse=True)
        j1, j2 = vAllJets[0], vAllJets[1]
        deta_all = abs(j1.Eta() - j2.Eta())
        mjj_all = (j1 + j2).M()
        
        # VBF veto: if delta eta <= 2.2 OR mjj <= 450, fail VBF (this matches Script 2)
        if deta_all <= 2.2 or mjj_all <= 450:
            continue
        
        # Now for VBF SELECTION, use ONLY light-flavor jets (matching Script 2's VBFjj_sel)
        if len(vJetsLF) < 2:
            continue
        
        # Sort light-flavor jets and take two leading
        vJetsLF.sort(key=lambda j: j.Pt(), reverse=True)
        q1, q2 = vJetsLF[0], vJetsLF[1]
        
        # VBF selection cuts (requires BOTH cuts to pass)
