...
class VBFx4bProcessor(processor.ProcessorABC):
    ...
    def _trigger_mask(self, events):
        ...
        n_jht  = int(ak.sum(fired_jht))
        n_btag = int(ak.sum(fired_btag))
        print(f"  [trigger] JetHT fired: {n_jht}  BTagCSV fired: {n_btag}  (in {len(events)} events)")
    ...
    def process(self, events):
        ...
        print(f"  after golden JSON  : {len(events)}")
        if len(events) == 0:
            return {}
        ...
        try:
            ...
            print(f"  WARNING: noise flags failed ({exc}), skipping noise filter")
        self.cnt["pass_noise"] += len(events)
        print(f"  after noise flags  : {len(events)}")
        if len(events) == 0:
            return {}
        ...
        print(f"  after trigger      : {len(events)}")
        if len(events) == 0:
            return {}
        ...
        # Cutflow sub-breakdown (useful for debugging)
        fj = events.FatJet
        fj_pt_f  = self._pt_field(fj)
        fj_sd    = self._msoftdrop_field(fj)
        n_base   = int(ak.sum(ak.any((fj_pt_f > 250) & (abs(fj.eta) < 2.4) & (fj.jetId == 6) & (fj_sd > 20), axis=1)))
        n_xbb    = int(ak.sum(ak.any(fj.particleNetMD_XbbvsQCD > 0.75, axis=1)))
        print(f"  fatjet: evts w/ base-pass jet={n_base}, w/ xbb jet={n_xbb}")
        ...
        print(f"  after fatjet sel   : {len(events)}")
        if len(events) == 0:
            return {}
        ...
        print(f"  lepton veto removes: {int(ak.sum(~lep_veto))} events")
        ...
        print(f"  after lepton veto  : {len(events)}")
        if len(events) == 0:
            return {}
        ...
        print(f"  after MET veto     : {len(events)}")
        if len(events) == 0:
            return {}
        ...
        print(f"  events with b-jet  : {int(ak.sum(has_bjet))}")
        ...
        print(f"  nlf>=2             : {int(ak.sum(nlf >= 2))}")
        print(f"  deta_sel>2.2       : {int(ak.sum(deta_sel > 2.2))}")
        print(f"  mjj_sel>450        : {int(ak.sum(mjj_sel > 450))}")
        print(f"  vbf_veto_fails     : {int(ak.sum(vbf_veto_fails))}")
        ...
        print(f"  after VBF sel      : {len(events)}")
        if len(events) == 0:
            return {}
        ...
        print(f"  x4b score range: min={float(ak.min(x4b)):.4f}  max={float(ak.max(x4b)):.4f}")
        print(f"  SR (>0.96): {int(ak.sum(is_SR))}  SB (>0.84): {int(ak.sum(is_SB))}")
...
def process_dataset(dataset_name, output_name=None):
    ...
    proc = VBFx4bProcessor(year, primary_dataset, lumi_mask, output_name)

    # Iterate over input files chunk by chunk
    for fpath in input_files:
        ...
        try:
            ...
            print(f"  Warning: failed to process {fpath}: {exc}")
            continue

    proc.flush_output()
    proc.print_summary(dataset_name)
    return True
...
def main():
    ...
    for ds in args.datasets:
        process_dataset(ds, args.output)
