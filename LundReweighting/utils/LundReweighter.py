        ...
        for i in range(len(out['reclust_nom'])):
            ...
            if(do_sys_weights):
                ...
                if(gen_parts_pdg_ids is None): gen_bs = []
                    ...
                    if len(gen_bs) == 0 or skip_bquark_unc:
                        b_rw = 1.0
                    else:
                        eta_phi = np.array(gen_parts_eta_phi[i])  # This event only (ignore shape irregularities)
        
                        #print("Shape of eta_phi:", np.shape(eta_phi))  # Debugging
                        #print("Shape of reclust_nom.subjet before call:", np.shape(reclust_nom.subjet))  # Debugging
                        #print("Contents of reclust_nom.subjet:", reclust_nom.subjet)  # Debugging

                        if len(reclust_nom.subjet) == 0: 
                            #print("reclust_nom.subjet is empty, skipping distance calculation.")
                            b_rw = 1.0  
                        else:
